...
public class MerchantController {
    ...
    public Response addMerchant(InputStream inputStream, @HeaderParam("Authorization") String basicAuthHeader) {
        JsonObject requestJson = Json.createObjectBuilder(Util.convertInputStreamToJson(inputStream))
                .add("requestType", RequestTypes.CREATE_MERCHANT.name())
                .build();
        return engine.routeRequest(requestJson, Modules.MERCHANT.name(), basicAuthHeader);
    }
